...
def make_continuous_dynamics(
    ...
    return Dynamics(
        spaces.Box(-1., 1., (1,), dtype=numpy.float32),
        numpy.array(limits),
        lambda action:
            numpy.multiply(numpy.clip(action, -1, 1), numpy.float32(speed)) *
            numpy.array([0., 1.], dtype=StateElement))
